...
class ScheduleStudentRes(MainRes):
    # Получение расписания студента на неделю
    def post(self):
        ...
        json_model = self.api_connector.getStudentScheduleByHash(acc_id, dates[0], dates[1])
        if json_model:
            self.app_metrica_reporter.sendEvent(event_name="StudentSchedule", event_json={
                "hash": hash_user,
                "acc_id": acc_id,
            })
