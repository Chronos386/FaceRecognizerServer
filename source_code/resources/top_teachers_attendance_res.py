...
class TopTeachersAttendanceRes(MainRes):
    def post(self):
        ...
        json_model = self.api_connector.getTopTeachersAttendance(dates[0], dates[1])
        if json_model:
            self.app_metrica_reporter.sendEvent(event_name="TopTeachersAttendance", event_json={
                "hash": hash_admin,
                "acc_id": acc_id,
            })
